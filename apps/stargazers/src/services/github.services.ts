...
enum GithubSearchApi {
  ...
  USER = '/users?q=',
  REPOS = '/repositories?q=',
}

export async function getGithubUsers(user: string): Promise<Response> {
  return fetch(`${GithubSearchApi.ROOT}${GithubSearchApi.USER}${user}`);
}

export async function getGithubUserRepos({ user, pageLimit, page, order }) {
